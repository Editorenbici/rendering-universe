...
\figplaceholder{3}{$\varepsilon$ vs.\ $\sqrt{\rho}R^{2}$ collapsing to a
single curve; manifoldlike vs.\ random-order separation (Exps 24/22).}
...
For a weakly disformal rescaling of a metric---clocks scaled by $\lambda^{-1}$
and rulers by $\lambda^{+1}$, i.e.\
...
\begin{enumerate}
\item \textbf{Biased insertion in 3+1 (in preparation):} a real 3+1 sprinkling
  with growth biased by a local resolution field, testing whether the
  $(-1,+3)$ split of Section~\ref{sec:sectors} emerges, with antichain widths
  estimated by the causal-overlap method of Bogu\~n\'a \& Krioukov
  \cite{BogunaKrioukov2024} rather than by raw adjacency---the
...
\bibitem{Surya2019} S.~Surya, \emph{The causal set approach to quantum
  gravity}, Living Rev.\ Relativ.\ \textbf{22}, 5 (2019).
\bibitem{BB1991} B.~Bollob\'as, G.~Brightwell, \emph{The height of a random
  partial order: concentration of measure}, Ann.\ Appl.\ Probab.\ (1992).
  % [author check: exact title/volume]
\bibitem{BrightwellLuczak} G.~Brightwell, M.~\L uczak, arXiv:1510.05612.
