...
\cite{GlaserSurya2013}---in a 4D causal diamond at matched density. Our result
agrees with the published value to $0.07\sigma$.
...
\end{equation}
The exponent is measured as $2.089 \pm 0.025$ and the amplitude matches
$\pi\sqrt{6}$ at the 5\% level with no fitted parameters. The $T^{2}$ is an
...
\figplaceholder{1}{$v$ vs.\ $T$: 2D logarithmic law and 4D area law with the
parameter-free theory curves superposed. To be generated from existing
outputs (Exps 18a/18b).}
...
\figplaceholder{2}{FRW amplitude ratios converging to $\mathfrak{F}$, with the
$1/\eta$ transient fit shown and subtracted (Exps 18c/18d).}
...
\begin{equation}
  ...
  \;=\; \frac{3\sqrt{6}}{\sqrt{\rho}\,R^{2}},
\end{equation}
which follows from the area law and the $R^{4}$ volume growth of the past
interval; the measured value tracks the prediction with ${\approx}10\%$
boundary corrections at our scales. The headline reproduction is bit-exact
...
\section{Three-sector accounting: a partition identity}
\label{sec:sectors}

The counting objects of this paper organize into three sectors: chains
(timelike depth $h$), antichains (spacelike width $w$), and links (the bonds).
For a weakly disformal rescaling of a metric \cite{Bekenstein1993}---clocks
scaled by $\lambda^{-1}$ and rulers by $\lambda^{+1}$, i.e.\
$ds^{2} = -\lambda^{-2}c^{2}dt^{2} + \lambda^{+2}dx^{2}$ to first order---the
sector counts of a faithful sprinkling must respond as
\begin{equation}
  \big(\text{chains} \times \lambda^{-1}\big) \times
  \big(\text{antichains} \times \lambda^{+3}\big)
  \;=\; \sqrt{-g} \;=\; \lambda^{+2}
\end{equation}
in 3+1 dimensions: the depth of maximal chains scales with proper time
($\lambda^{-1}$), the width of maximal antichains with spatial volume
($\lambda^{+3}$), and their product must recover the volume element, which is
what a Poisson sprinkling actually samples. The identity closes:
$-1 + 3 = +2$. A \emph{conformal} rescaling (clocks and rulers both
$\lambda^{+1}$) requires the chain sector to absorb $\lambda^{-4}$ for the
same volume budget---the opposite sign, and countable.

We stress the epistemic status: this is a \textbf{consistency theorem about
counting}, not a dynamical statement. It says that \emph{if} a poset's growth
is biased so as to mimic a disformal metric, its chain and antichain sectors
must split the volume budget as $(-1, +3)$; it does not say any natural growth
dynamics does so. Whether biased sequential growth can produce these exponents
is an open, pre-registered question (Section~\ref{sec:outlook}). A first toy
experiment in 1+1 dimensions (where the disformal split is $(-1,+1)$ and there
is no surplus to allocate) found the width sector saturating its budget and
the depth sector near zero, with a smooth sign crossover as the bias
increases---a susceptibility-like response, not a phase transition---but its
dimensional targets were mismatched by design and it decides nothing beyond
the toy.

\figplaceholder{4}{Three-sector schematic: chains (depth), antichains (width),
links (bonds), and the $(-1,+3) \to +2$ volume budget.}
...
\begin{enumerate}
\item \textbf{Validation in a disformal metric (pre-registered):} a uniform
  sprinkling \emph{into} the weakly disformal geometry
  $ds^{2} = -\mathcal{R}^{-2}dt^{2} + \mathcal{R}^{2}d\vec{x}^{\,2}$, where
  the partition identity of Section~\ref{sec:sectors} predicts $(-1,+3)$
  analytically. The experiment validates that the identity survives finite
  size and boundaries and that windowed local estimators---depth in
  proper-radius balls, width by the causal-overlap method of Bogu\~n\'a \&
  Krioukov \cite{BogunaKrioukov2024} rather than raw adjacency, which is
  asymptotically silent---can \emph{see} the signature. Chain lengths in
  inhomogeneous (Schwarzschild) sprinklings have been studied
  \cite{Eichhorn2026}; the disformal partition test appears to be new.
  Whether biased \emph{growth dynamics} produce this split is a separate,
  open question deferred to future work.
