...
$-1 + 3 = +2$. The \emph{conformal} counterpart (clocks and rulers scaled
together, $\lambda^{-1}$ each in this convention) gives
$\sqrt{-g} = \lambda^{-4}$ with both sectors contracted
($\lambda^{-1} \times \lambda^{-3}$): a \emph{deficit} of elements where the
disformal case has a surplus. The two assignments thus make opposite-sign,
countable predictions for the sector split of a faithful sprinkling.
