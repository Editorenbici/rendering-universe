...
result---and the pre-registered test of its exponents, which our control
battery found to be instrument-limited in diamond geometry: a ghost-field
control exposed a radial confound that a fiducial re-measurement reduced
fivefold without eliminating, leaving the identity untested rather than
refuted, with the homogeneous-domain redesign defined. All results are
...
\begin{enumerate}
\item \textbf{Estimator test in a disformal metric (pre-registered, run,
  instrument-limited):} we sprinkled uniformly \emph{into} the weakly
  disformal geometry
  $ds^{2} = -\mathcal{R}^{-2}dt^{2} + \mathcal{R}^{2}d\vec{x}^{\,2}$, where
  the partition identity above predicts $(-1,+3)$ analytically, measuring
  depth in proper-radius balls and width by the causal-overlap method of
  Bogu\~n\'a \& Krioukov \cite{BogunaKrioukov2024} (raw adjacency is
  asymptotically silent). The pre-registered control battery caught the
  measurement twice: a ghost-field control (the same fit on a \emph{flat}
  sprinkling against a decoupled $\mathcal{R}$ profile, which must return
  zero) gave exponents of the same size as the signal, exposing a
  radial confound of the diamond geometry---its center simply has more
  accessible depth and more neighbours than its boundary region. A
  pre-registered fiducial-region re-measurement reduced the ghost fivefold
  but did not eliminate it, and a positive control with an analytically
  known answer still failed. We therefore report the diamond geometry as
  \emph{instrument-limited} for windowed depth/width estimators: the
  partition identity remains untested (not refuted), and the redesign---a
  translationally homogeneous domain with no privileged centre---is
  defined. All runs, pre-registrations, amendments, and control outputs
  are public. Chain lengths in inhomogeneous (Schwarzschild) sprinklings
  have been studied \cite{Eichhorn2026}; the disformal partition test, and
  the ghost-field methodology that disciplined it, appear to be new.
