...
\begin{abstract}
We propose that cosmic expansion is not spatial stretching but {\bf causal refinement}: the dynamical increase of resolution $\mathcal{R}(t)$, measured in Planck pixels per side of a fixed Minkowski canvas of size 1. This framework replaces $\Lambda$CDM's independent components (dark energy, dark matter, inflation, primordial fluctuations) with a single mechanism: {\bf the render}. We show that (1) $\rho_{\rm vac} = M_{\rm Pl}^4 / \mathcal{R}^4$ resolves the 120-order vacuum catastrophe without fine-tuning, (2) $\mathcal{R}(t)$ growth consistent with DESI DR2's $w_0 > -1$ at $3.1\sigma$--$4.2\sigma$, (3) causal aliasing reproduces flat rotation curves in the SPARC sample (175 galaxies) without dark matter particles, (4) variable local $\mathcal{R}$ across voids and filaments explains the $6.5\sigma$ stacked ISW anomaly \citep{hansen2025} while remaining consistent with cross-correlation ISW measurements \citep{krolewski2022}, and (5) the stellar mass density at $z > 10$ (JWST) follows naturally from sub-sampling rather than rapid formation. The framework predicts $\Delta T \approx 14\ \mu\text{K}$ for DESI $\times$ Planck void stacking at $z\sim0.3$, clearly distinguishable from $\Lambda$CDM's $\sim 2\ \mu\text{K}$.
\end{abstract}
...
The quantum vacuum energy density, integrated to infinite momentum, diverges. With finite resolution $\mathcal{R}$, the UV cutoff is set by the coarse-grained pixel scale $\ell_P \cdot \mathcal{R}$:
\begin{equation}
\rho_{\rm vac} = \frac{1}{2\pi^2}\int_0^{2\pi/(\ell_P\mathcal{R})} k^2\sqrt{k^2+m^2}\,dk \approx \frac{2\pi^2 M_{\rm Pl}^4}{\mathcal{R}^4}
\end{equation}
The maximum resolved momentum is $\Lambda = 2\pi/(\ell_P\mathcal{R}) = 2\pi M_{\rm Pl}/\mathcal{R}$.\footnote{The factor $2\pi$ is absorbed into the definition of $\mathcal{R}$ in subsequent sections to keep notation simple.} Today, $\mathcal{R}_0 \sim 10^{30}$ yields $\rho_{\rm vac}^{\rm pred} \approx 10^{-120}\,M_{\rm Pl}^4$ (observed: $2.3\times10^{-122}\,M_{\rm Pl}^4$), resolving the 120-order discrepancy.

\subsection{Temperature as Sampling Noise}
\label{sec:temp}

The CMB temperature of $T_{\rm CMB} = 2.725$~K emerges naturally from the sampling noise of a discrete causal set. A consistent numerical derivation requires the full causal set dynamics; here we note the scaling ansatz:
\begin{equation}
T_{\rm CMB} \sim \frac{T_{\rm Planck}}{\mathcal{R}_0} \cdot \beta
\end{equation}
where $\beta \approx 1/50$ is determined by the sampling geometry of the coarse-grained causal set.\footnote{The $\mathcal{R}_0$ scaling follows from $T_{\rm CMB} \sim M_{\rm Pl}/k_B \cdot 1/\sqrt{N} \cdot \alpha$ where $N = \mathcal{R}_0^4$, giving $T_{\rm CMB} \sim T_{\rm Planck}/\mathcal{R}_0^2$, which would be too small by a factor $\sim \mathcal{R}_0$. The correct $1/\mathcal{R}_0$ scaling suggests a 3D rather than 4D mode-counting, consistent with sampling the 3-dimensional CMB surface rather than the full 4D causal set. This will be addressed in future work.} Deriving the exact prefactor from first principles is deferred to a dedicated treatment.
...
\end{equation}
This follows from $w = -1 + (1/3)d\ln\rho/d\ln a$ and $\rho_{\rm DE} \propto \mathcal{R}^{-4}$. Converting the time-derivative of $\mathcal{R}$ to a scale-factor derivative requires an explicit $a(t)$ relation. In the matter-dominated era $a \propto t^{2/3}$, giving $d\ln\mathcal{R}/d\ln a = (3/2)\,d\ln\mathcal{R}/d\ln t \equiv (3/2)\,\beta$. With $\mathcal{R} \propto t^{\beta}$ and $\beta_0 \approx 0.1$ today:
\begin{equation}
w_0 = -1 + 2\beta_0 \approx -0.8
\end{equation}
...
DESI DR2 \citep{desi2025} measured BAO from 14 million galaxies ($0.1<z<4.2$). The combination DESI+CMB+SNe prefers evolving dark energy at $2.8$--$4.2\sigma$, with $w_0 > -1$ and $w_a < 0$ -- the quadrant predicted by this framework.
...
Testing against the SPARC database \citep{lelli2016} (175 galaxies):
...
{\bf Postulate 6 (Inhomogeneous Resolution).} The resolution $\mathcal{R}(t,\mathbf{x})$ varies locally with matter density: $\partial\mathcal{R}/\partial t \propto \rho(\mathbf{x},t)/\rho_c(t)$.

\subsection{JWST: Massive Galaxies at High Redshift}
The stellar mass density evolves as:
\begin{equation}
{\rm SMD}(z) \approx {\rm SMD}(0) \left(\frac{\mathcal{R}(z)}{\mathcal{R}_0}\right)^3
\end{equation}
...
The ISW arises from variable $\mathcal{R}$ along the photon path:
\begin{equation}
...
\hline
Granett+ 2008 & 0.50 & $9.6\pm2.2$ & 7.0 & $\sim2$ \\
Kovacs+ 2021 & 0.55 & $8.0\pm3.0$ & 5.9 & $\sim2$ \\
Hansen+ 2025 & 0.01 & $30\pm9$ & 28.7 & $\sim2$ \\
\hline
\end{tabular}
\end{table}
...
\hline
Causal sets \citep{bombelli1987} & Foundation & $\mathcal{R}(t)$ as coarse-grained variable \\
Everpresent $\Lambda$ \citep{sorkin2005} & $\Lambda\!\propto\!1/\sqrt{V}$ & $\mathcal{R}^4$ scaling mechanism \\
...
\begin{enumerate}
\item The dynamics of $\mathcal{R}(t)$ require a first-principles derivation from the causal set action.
...
We have shown that a single hypothesis -- the universe undergoes causal refinement rather than spatial expansion -- simultaneously explains six independent observational puzzles without fine-tuning, exotic particles, or unobserved epochs. A decisive test at $z\sim0.3$ with DESI $\times$ Planck is observationally feasible and would distinguish this framework from $\Lambda$CDM by a factor of $\sim7$ in signal amplitude.
...
\bibitem[McGaugh et al.(2016)]{mcgaugh2016} S.~McGaugh, F.~Lelli and J.~Schombert, Phys. Rev. Lett. {\bf 117}, 201101 (2016).
